...
# --------------------------------------------------------------------------
# The call site: flags raised through the insert path
# --------------------------------------------------------------------------
...
class _Connection:
    """Enough of a Connection for commit_draft, with a fixed exercises table."""
    ...
    def execute(self, statement, params=None):
        if "FROM exercises" in str(statement):
            return _Result([(name, index + 1) for index, name in enumerate(self.existing)])
        return _Result([])
...
def commit_one(name, raw_text, existing=(), edited_fields=frozenset(), added=False):
    ...
    draft = pipeline.draft_from_payload(
        {"sets": [{"exercise_name": name, "weight_kg": 40.0, "reps": 8,
                   "raw_span": raw_text}]},
        raw_text,
        date(2026, 8, 22),
    )
...
class TestFlagsReachTheResult:
    """`commit_draft` is the only function that inserts, so it raises the flags.

    The checks themselves are covered above against the pure function; these
    cover the wiring, which is the part that silently stops working.
    """

    def test_a_new_exercise_with_a_typo_is_flagged(self):
        result = commit_one("Bech Press", "bech press 40kg 8",
                            existing=["Chest Bench Press"])
        assert [flag.reason for flag in result.name_flags] == ["near_miss"]
        assert result.name_flags[0].nearest_name == "Chest Bench Press"

    def test_a_flagged_row_is_still_saved(self):
        """The flag is amber, not a rejection - it must never withhold data."""
        result = commit_one("Bech Press", "bech press 40kg 8",
                            existing=["Chest Bench Press"])
        assert result.inserted_sets == 1
        assert result.review_items == []

    def test_an_ordinary_new_exercise_is_not_flagged(self):
        result = commit_one("Barbell Squat", "barbell squat 40kg 8")
        assert result.name_flags == []

    def test_an_exercise_that_already_exists_is_not_flagged(self):
        """Re-logging a known lift creates nothing, so there is nothing to flag."""
        result = commit_one("Chest Bench Press", "chest bench press 40kg 8",
                            existing=["Chest Bench Press"])
        assert result.name_flags == [] and result.inserted_sets == 1

    def test_a_name_typed_by_the_user_is_not_grounding_flagged(self):
        """No extraction to score: the user wrote the name, the entry did not.

        The same reason `commit_draft` pins confidence to 1.0 on an edited row.
        Left as an extraction, this name is weakly grounded and would flag.
        """
        raw = "third machine in the corner, 40kg 8"
        assert commit_one("Pec Deck", raw).name_flags[0].reason == "weak_grounding"
        assert commit_one("Pec Deck", raw,
                          edited_fields=frozenset({"exercise_name"})).name_flags == []

    def test_a_name_typed_by_the_user_is_still_near_miss_flagged(self):
        """A typo splits history no matter who typed it, so that check stays on."""
        result = commit_one("Bech Press", "bech press 40kg 8",
                            existing=["Chest Bench Press"],
                            edited_fields=frozenset({"exercise_name"}))
        assert [flag.reason for flag in result.name_flags] == ["near_miss"]
